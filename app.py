...
def inject_timeline():
    ...
    timeline_msg = ""
    if not is_open:
        if active_reason and active_reason not in ["ไม่อยู่ในช่วงเวลาการเปิดรับคำขอ", "ยังไม่มาเปิดรับคำขอใหม่"]:
...
def translate_work_type(initial_type):
    ...
    return wt['label'] if wt else (str(initial_type) or '-')


@app.template_filter('format_score')
def format_score(val):
    """แสดงคะแนน ทศนิยม 2 ตำแหน่ง เลี่ยง 12.000 ให้อ่านว่า 12.00"""
    if val is None:
        return '-'
    try:
...
@app.template_filter('date_table')
def date_table(date_str):
    """แสดงวันที่แบบสั้นสำหรับตาราง (ตัดเวลา 00:00 ออก) รองรับทั้ง DD/MM/YYYY และ D ม.ย. YYYY"""
...
def request_entity_too_large(error):
    ...
    <head>
        ...
        <style>
            body {{ font-family: 'Kanit', sans-serif; display: flex; justify-content: center; align-items: center; height: 100vh; background-color: #f4f6f9; margin: 0; }}
            ...
            p {{ color: #475569; font-size: 16px; margin: 15px 0; line-height: 1.5; }}
            .size-badge {{ display: inline-block; background-color: #fee2e2; color: #dc2626; padding: 8px 15px; border-radius: 20px; font-weight: bold; margin: 15px 0; }}
            .btn {{ display: inline-block; margin-top: 20px; padding: 10px 25px; background: #3b82f6; color: white; text-decoration: none; border-radius: 8px; font-weight: 600; transition: all 0.2s; }}
