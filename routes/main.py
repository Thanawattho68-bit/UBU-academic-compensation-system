...
def read_notification(notif_id):
    ...
    # ตรวจสอบว่าแจ้งเตือนนี้เป็นของผู้ใช้ปัจจุบัน
    notif = query_db('SELECT * FROM Notification WHERE id = ?', (notif_id,), one=True)
    if notif and (notif['recipient_username'] == session['username'] or notif['recipient_role'] == session['role']):
